...
def main(args):
    ...
    for root, directories, files in os.walk(image_folder, topdown=False):
        if len(root) > prefix_len and (root[-1] == "1" or root[-1] == "2" or root[-1] == "HR_1") :
            ...
            if args.isTest :
                ...
                save_folder_list.append(root.replace('train_release', 'train_release_out'))
    ...
    for f in range(0, len(image_folder_list)):
        image_path_list = []
        for files in types:
            image_path_list.extend(glob(os.path.join(image_folder_list[f], files)))
        ...
        for i, image_path in enumerate(image_path_list):
            ...
            if args.isImage:
                imsave(os.path.join(save_folder_list[f], name + '.jpg'), image)
            ...
            if args.is3d:
                ...
                if args.isTexture:
                    ...
                    write_obj_with_texture(os.path.join(save_folder_list[f], name + '.obj'), save_vertices, prn.triangles, texture, prn.uv_coords/prn.resolution_op)#save 3d face with texture(can open with meshlab)
                else:
                    write_obj_with_colors(os.path.join(save_folder_list[f], name + '.obj'), save_vertices, prn.triangles, colors) #save 3d face(can open with meshlab)

            if args.isDepth:
                depth_image = get_depth_image(vertices, prn.triangles, h, w, True)
                #depth = get_depth_image(vertices, prn.triangles, h, w)
                imsave(os.path.join(save_folder_list[f], name + '_depth.jpg'), depth_image)
                #sio.savemat(os.path.join(save_folder_list[f], name + '_depth.mat'), {'depth':depth})

            if args.isMat:
                sio.savemat(os.path.join(save_folder_list[f], name + '_mesh.mat'), {'vertices': vertices, 'colors': colors, 'triangles': prn.triangles})

            if args.isKpt or args.isShow:
                # get landmarks
                kpt = prn.get_landmarks(pos)
                np.savetxt(os.path.join(save_folder_list[f], name + '_kpt.txt'), kpt)

            if args.isPose or args.isShow:
                # estimate pose
                camera_matrix, pose = estimate_pose(vertices)
                np.savetxt(os.path.join(save_folder_list[f], name + '_pose.txt'), pose) 
                np.savetxt(os.path.join(save_folder_list[f], name + '_camera_matrix.txt'), camera_matrix) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--inputDir', default='../casia-fasd-frames/test_release/', type=str,
                        help='path to the input directory, where input images are stored.')
    parser.add_argument('--isTest', default=True, type=ast.literal_eval,
                        help='whether to generating for test_release folder')
